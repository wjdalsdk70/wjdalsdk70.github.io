...
export default function RightSidebar({ recentPosts, songUrl }: RightSidebarProps) {
  ...
  return (
    <aside className="hidden xl:flex w-80 min-h-screen bg-[var(--sidebar-bg)] flex-col px-6 pt-8 pb-4 border-l border-[var(--border)] sticky top-0 h-screen overflow-y-auto">
        ...
        <ul className="space-y-4">
          {recentPosts.map((post) => (
            <li key={post.slug} className="flex flex-col gap-1">
              ...
              <time className="text-xs text-[var(--text-muted)]" dateTime={post.date}>
